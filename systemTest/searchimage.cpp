#ifndef SEARCHIMAGE_C
#define SEARCHIMAGE_C
...
//			{aimage=512;}
		dxl_write_word(19,30,aimage);
		dxl_write_word(20,30,bimage);
		if(bimage<=570&&bimage>=480){comimage = 1;}
		else if(bimage>570){comimage = 2;}
		else if(bimage<480){comimage = 3;}
}
...
void cari(){
	pimage2=0;
	if(aimage>450 || aimage<300 || bimage>650 || bimage<350){
	aimage=400;
	bimage=512;
	}
	 for (int i=bimage; i<=650; i=i+10){
		 dxl_write_word(20,30,i);
//		 dxl_write_word(19,30,aimage); 
		usleep(limage);
		 awal();
		 bb=i;
	 ...
	 for (int j=aimage; j>=300; j=j-10){
		dxl_write_word(19,30,j);
		usleep(limage);
	...
	for (int i=tempimage; i<=520; i=i+10){
		dxl_write_word(20,30,i);
		usleep(limage);
